      ...
      <div className="flex-1 overflow-auto bg-white px-2 sm:px-4 md:px-6">
        <table className="min-w-full text-[11px] sm:text-xs md:text-sm">
          <thead className="bg-gray-100 sticky top-0 z-10">
            <tr>
              <th className="px-2 sm:px-3 py-2 text-center font-medium text-gray-600">운행날짜</th>
              <th className="px-2 sm:px-3 py-2 text-center font-medium text-gray-600">차량</th>
              <th className="px-2 sm:px-3 py-2 text-center font-medium text-gray-600">운전자</th>
              <th className="px-2 sm:px-3 py-2 text-center font-medium text-gray-600">운행거리</th>
              ...
              <th className="px-2 sm:px-3 py-2 text-center font-medium text-gray-600">경유지</th>
              <th className="px-2 sm:px-3 py-2 text-center font-medium text-gray-600">이중주차</th>
              <th className="px-2 sm:px-3 py-2 text-center font-medium text-gray-600">운행일지</th>
            </tr>
          </thead>
          <tbody>
            {filteredLogs.length === 0 ? (
              <tr>
                <td colSpan={9} className="px-3 py-8 text-center text-gray-400 text-xs sm:text-sm">
                  해당 월의 운행일지가 없습니다.
                </td>
              </tr>
            ) : (
              filteredLogs.map((log) => {
                ...
                return (
                  <tr
                    key={log.id}
                    className={`${canOpenLogForm ? 'cursor-pointer' : ''} ${
                  ...
                  >
                    ...
                    <td className="px-2 py-2 text-center whitespace-nowrap">
                      {log.driver || booking?.requester || booking?.userName || '-'}
                    </td>

                    <td className="px-2 py-2 text-center whitespace-nowrap">{distanceDisplay}</td>

                    <td className="px-2 py-2 text-center whitespace-nowrap">{finalKmDisplay}</td>
                    ...
                    <td className="px-2 py-2 w-[170px]">{log.via || '미입력'} </td>
