...
export default function VehicleApp() {
  ...
  useEffect(() => {
    ...
    setFormData((prev) => ({
      ...prev,
      requester: prev.requester || user.displayName || user.email || user.uid || user.role || '',
    }));
  }, [user]);
  ...
  useEffect(() => {
    ...
    const existingLog = driveLogs.find((log) => log.bookingId === booking.id);

    setLogForm({
      from: (existingLog && existingLog.from) || '다남프라자',
      via: (existingLog && existingLog.via) || '',
      to: (existingLog && existingLog.to) || '다남프라자',
      finalKm: existingLog && existingLog.finalKm != null ? String(existingLog.finalKm) : '',
      purpose: (existingLog && existingLog.purpose) || booking.purpose || '',
      driver: (existingLog && existingLog.driver) || booking.requester || booking.userName || '',
  ...
  const handleFabClick = async () => {
    if (user?.email) {
      try {
        const userDocRef = doc(db, `artifacts/${appId}/public/data/allowed_users/${user.email}`);
        const userDocSnap = await getDoc(userDocRef);

        if (userDocSnap.exists()) {
          console.log('현재 사용자 Firestore 정보:', userDocSnap.data());
        } else {
          console.log('allowed_users 에 현재 사용자 문서가 없습니다. email:', user.email);
        }
      } catch (error) {
    ...
    setFormData({
      ...
      destination: '',
      purpose: '',
      requester: user?.displayName || user?.email || user?.uid || user?.role || '',
  ...
  const handleBookingSubmit = async () => {
    ...
    if (!normalizedStart || !normalizedEnd) {
      toast.error('시간 형식이 올바르지 않습니다.', {
        description: '출발/도착 시간을 예: 09:00, 9-00, 900 와 같은 형식으로 입력해 주세요.',
      });
    ...
    const excludeId = formMode === 'edit' && selectedBooking ? selectedBooking.id : undefined;

    if (utilCheckOverlap(bookings, formData.vehicleId, dateStr, startTime, endTime, excludeId)) {
  ...
  const handleLogSubmit = async () => {
    ...
    if (prevKm != null && finalKm < prevKm) {
      toast.error('키로수 입력이 올바르지 않습니다.', {
        description: '현재 최종 키로수는 이전 최종 키로수보다 크거나 같아야 합니다.',
      });
      return;
    }

    const distanceKm = prevKm != null ? finalKm - prevKm : 0;
    ...
    try {
      ...
      if (existingLog) {
        ...
        await addDoc(
          collection(db, 'artifacts', String(appId), 'public', 'data', 'vehicle_drive_logs'),
          {
            ...baseData,
            createdAt: new Date().toISOString(),
          } as Omit<DriveLog, 'id'>
  ...
  const headerTitle = isLogView ? '차량운행일지' : view === 'user' ? '내 이용내역' : '차량신청현황';
  ...
  const pendingLogBadgeText = useMemo(
    () => (pendingLogCount > 9 ? '9+' : pendingLogCount.toString()),
    [pendingLogCount]
  );
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-100 px-0 sm:px-2 md:px-4 lg:px-8">
      ...
      {/* FAB */}
      <FAB visible={fabVisible} onClick={handleFabClick} />
    </div>
  );
}
